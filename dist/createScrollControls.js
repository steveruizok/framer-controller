 ...
 * ```tsx
  ...
  ```*/
function createScrollControls(options = {}) {
    const { scrollY = 0, scrollX = 0 } = options;
    const contentOffsetX = framer_1.motionValue(-scrollX);
    const contentOffsetY = framer_1.motionValue(-scrollY);
    // Initial Store
    let store = {
        component: null,
        content: null,
    ...
    const setStoreState = (changes, updateMotion) => {
        store = Object.assign({}, store, changes);
        store = Object.assign({}, store, { scrollPoint: {
                x: store.scrollX,
                y: store.scrollY,
            }, progress: {
                x: store.scrollX /
    ...
    const set = (values, updateMotion = true) => setStoreState(values, updateMotion);
    // Update from contentOffsetY
    const updateFromMotionValues = (scrollX, scrollY, delta, offset, velocity) => {
        set({
            scrollX,
            scrollY,
            progress: {
...
```*/
     ...
     * - progress,
     */
    const useScrollItemState = (props) => {
        ...
        const calculateClip = (axis, a, b, max) => {
            ...
            setState((state) => {
                ...
                const travel = {
                    x: {
                        before: 1,
                        after: -1,
                        right: framer_1.transform(offset.left, [sf.width, sf.width - absolute.width], [-1, 0]),
                        left: framer_1.transform(offset.left, [0, -absolute.width], [0, 1]),
                        overflow: 0,
                        contain: 0,
                    }[clip.x],
                    y: {
                        before: 1,
                        after: -1,
                        bottom: framer_1.transform(offset.top, [sf.height, sf.height - absolute.height], [-1, 0]),
                        top: framer_1.transform(offset.top, [0, -absolute.height], [0, 1]),
                        overflow: 0,
                        contain: 0,
                    }[clip.y],
                };
                const progress = {
                    x: framer_1.transform(offset.left, [sf.width, -absolute.width], [0, 1]),
                    y: framer_1.transform(offset.top, [sf.height, -absolute.height], [0, 1]),
                };
        ...
        React.useEffect(() => {
            ...
            const isChild = store.content.props.children
                ...
                .includes(props.id);
            if (!isChild) {
                console.error("⚠️ You've passed the props of a component that isn't a child of the Scroll component's content Frame to useScrollControls. You should only pass props from a component that is a descendent of the Scroll's connected content Frame.");
                return;
            }
            const absolute = {
                x: props.left,
            ...
            setState(Object.assign({}, state, { absolute }));
        }, [store.content]);
        return state;
    };
    return {
...
exports.createScrollControls = createScrollControls;
